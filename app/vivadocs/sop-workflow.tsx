...
export function SopWorkflow({ onClose, onSaved }: { onClose(): void; onSaved?(): void }) {
  ...
  async function downloadPdf() {
    ...
    try {
      ...
      const margin = 17, width = 176, pageHeight = 297; let y = 17;
      ...
      for (const step of saved.steps) {
        pageBreak(35); pdf.setFont("helvetica", "bold"); pdf.setFontSize(12); pdf.setTextColor(42, 112, 201); pdf.text(`Step ${step.position}`, margin, y); y += 7;
        pdf.setFont("helvetica", "normal"); pdf.setFontSize(10); pdf.setTextColor(61, 66, 72);
        const lines = pdf.splitTextToSize(step.instruction, width); pageBreak(lines.length * 5 + 8); pdf.text(lines, margin, y); y += lines.length * 5 + 4;
        if (step.existingImageUrl) {
          const data = await imageAsDataUrl(step.existingImageUrl); const props = pdf.getImageProperties(data); const imageWidth = Math.min(width, 150); const imageHeight = Math.min(105, imageWidth * props.height / props.width); pageBreak(imageHeight + 12); pdf.addImage(data, props.fileType, margin, y, imageWidth, imageHeight); y += imageHeight + 4;
